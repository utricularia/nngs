...
extern struct plan * plan_init(void);

/* Returns the number of info lines in list. */
extern size_t plan_count(struct plan *p);
...
** }
*/
extern void plan_start(struct plan *p);

/* Gives the next planed in turn.
** Returns 1 if there was one, 0 if we reached the end.
*/
extern int plan_next(char **cp, struct plan *p);

/* Clear the list
*/
extern void plan_clear(struct plan *p);
